...
public class GridPasswordView extends LinearLayout implements PasswordView {
    ...
    private void initAttrs(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        itemWidth = ta.getDimensionPixelSize(R.styleable.gridPasswordView_gpv_item_width, 0);
        itemHeight = ta.getDimensionPixelSize(R.styleable.gridPasswordView_gpv_item_height, 0);
        spacing = ta.getDimensionPixelSize(R.styleable.gridPasswordView_gpv_spacing, 0);
        ...
        mOuterLineDrawable = ta.getDrawable(R.styleable.gridPasswordView_gpvBackGround);
    ...
    private void inflaterViews(Context context) {
        ...
        if (spacing != 0 && itemWidth != 0) {
            if (itemHeight == 0) {
                itemHeight = itemWidth;
            }
            LayoutParams layoutParams = new LayoutParams((int) itemWidth, (int) itemHeight);
            layoutParams.leftMargin = (int) (spacing / 2);
            layoutParams.rightMargin = (int) (spacing / 2);
            mInputView.setLayoutParams(layoutParams);
        }
        setCustomAttr(mInputView);
        mInputView.setBackground(mOuterLineDrawable);
        mViewArr[0] = mInputView;
        int index = 1;
        while (index < mPasswordLength) {
            TextView textView = (TextView) inflater.inflate(R.layout.textview, null);
            textView.setBackground(mOuterLineDrawable);
            setCustomAttr(textView);
            LayoutParams textViewParams;
            if (spacing != 0 && itemWidth != 0) {
                textViewParams = new LayoutParams((int) itemWidth, (int) itemHeight);
                textViewParams.leftMargin = (int) (spacing / 2);
                textViewParams.rightMargin = (int) (spacing / 2);
            } else {
                textViewParams = new LayoutParams(0, LayoutParams.MATCH_PARENT, 1f);
            }
